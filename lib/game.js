    ...
    for (var i = 0; i < this.allObjects.length - 1; i++) {
      for (var j = i + 1; j < this.allObjects.length; j++) {
        if (this.allObjects[i].collideWith(this.allObjects[j])) {
          ...
          switch (twoObj) {
            case (twoObj[0] instanceof Ship && twoObj[1] instanceof Solaroid):
              this.allObjects[i].relocate();
              break;
            case (twoObj[0] instanceof Solaroid && twoObj[1] instanceof Solaroid):
              this.shrinkObject(j);
              this.shrinkObject(i);
              break;
            case (twoObj[0] instanceof Beam && twoObj[1] instanceof Solaroid):
            case (twoObj[0] instanceof Solaroid && twoObj[1] instanceof Beam):
              twoObj[0].radius /= 2;
              twoObj[1].radius /= 2;
            }
